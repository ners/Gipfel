...
instance ToSchema GQLRequest where
    declareNamedSchema _ = do
        ...
        anySchema <- declareSchemaRef (Proxy @Any)
        pure $
            NamedSchema (Just "GQLRequest") $
                mempty
                    & type_ ?~ OpenApiObject
                    & properties
                        .~ [ ("query", textSchema)
...
instance ToSchema GQLResponse where
    declareNamedSchema _ = do
        let (NamedSchema _ validValue) = NamedSchema (Just "ValidValue") $ mempty & type_ ?~ OpenApiObject
        errorSchema <- declareSchemaRef $ Proxy @GQLError
